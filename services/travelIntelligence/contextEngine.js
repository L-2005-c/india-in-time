...
function createIntelligenceContext(opts = {}) {
  ...
  return {
    ...
    weather: {
      dataState: (weather && (weather.tempC != null || weather.condition)) ? (weather.freshness === 'observed' ? 'OBSERVED' : 'ESTIMATED') : 'UNAVAILABLE',
      condition: weather?.condition || 'Unknown',
      tempC: weather?.tempC ?? null,
      apparentTempC: weather?.apparentTempC ?? weather?.tempC ?? null,
      humidity: weather?.humidity ?? null,
      rainRisk: weather?.rainRisk || (/rain/i.test(weather?.condition || '') ? 'HIGH' : (weather ? 'LOW' : 'UNKNOWN')),
      windKph: weather?.windKph ?? null,
      status: weather?.status || (weather?.tempC != null ? (weather.tempC >= 40 || /storm|heavy rain/i.test(weather?.condition || '') ? 'POOR' : 'GOOD') : 'UNKNOWN'),
      freshness: weather?.freshness || 'unknown',
      isAvailable: Boolean(weather && weather.tempC != null),
    },
    crowd: {
      dataState: crowd?.level ? 'ESTIMATED' : 'UNKNOWN',
      level: crowd?.level || 'UNKNOWN',
      score: crowd?.score ?? crowd?.crowdScore ?? null,
      estimatedQueueMinutes: crowd?.estimatedQueueMinutes ?? null,
      queueDescriptor: crowd?.queueDescriptor || (crowd?.level ? null : 'Crowd estimate unavailable'),
      source: crowd?.source || PROVENANCE_SOURCES.HISTORICAL_CROWD,
      isAvailable: Boolean(crowd && crowd.level && crowd.level !== 'UNKNOWN'),
    },
    traffic: {
      dataState: traffic?.source === 'live' || traffic?.source === 'live_traffic'
        ? 'LIVE'
        : (traffic?.travelMinutes != null ? 'ESTIMATED' : 'UNAVAILABLE'),
      travelMinutes: traffic?.travelMinutes ?? null,
      freeFlowMinutes: traffic?.freeFlowMinutes ?? null,
      trafficDelayMinutes: traffic?.trafficDelayMinutes ?? null,
      ...
      source: traffic?.source === 'live' || traffic?.source === 'live_traffic'
        ...
        : (traffic?.travelMinutes != null ? PROVENANCE_SOURCES.STATIC_ROUTE_ESTIMATE : PROVENANCE_SOURCES.UNKNOWN),
      isAvailable: Boolean(traffic && traffic.travelMinutes != null),
    },
    route: {
    ...
    scenic: {
      score: scenic?.scenicScore ?? scenic?.score ?? null,
    ...
    openingHours: {
      dataState: openingHours?.status ? (openingHours.dataQuality === 'official' ? 'OFFICIAL' : 'ESTIMATED') : 'UNKNOWN',
      isOpen: openingHours?.isOpenNow ?? openingHours?.isOpen ?? null,
      status: openingHours?.status || 'UNKNOWN',
      minutesToClose: openingHours?.minutesToClose ?? null,
      isAvailable: Boolean(openingHours && openingHours.status && openingHours.status !== 'UNKNOWN'),
    },
    mealContext: {
