...
function getPreviewRouteStart(){
  return tripActive && cLat&&cLon ? [cLat,cLon] : getCityCenter();
}
...
function resetPlanUI(){
  ...
  document.getElementById('plan-list').innerHTML=`<div class="empty-state"><div class="empty-icon">🗺️</div><p class="empty-txt">Ready! Shape the experience and generate a polished plan for ${currentCityName}.</p><p class="empty-sub">Set the mood, confirm timing, then let AI stage the route.</p></div>`;
  updatePlannerShowcase();
}
  ...
  el.innerHTML = [
    '<div class="budget-card" style="margin-bottom:18px;background:linear-gradient(135deg,rgba(0,212,255,.08),rgba(0,212,184,.08));border-color:rgba(0,212,255,.15)">',
      '<div class="budget-row">',
        `<div><div class="inp-lbl">Trip snapshot</div><div class="tools-section-title" style="margin:4px 0 0;font-size:18px;letter-spacing:0;color:var(--text-primary);text-transform:none">${c}</div></div>`,
...
async function renderRoute(){
  ...
  try{const coords=raw.map(p=>`${p[1]},${p[0]}`).join(';');const res=await fetch(`https://routing.openstreetmap.de/routed-car/route/v1/driving/${coords}?overview=full&geometries=geojson&steps=true`,{signal:AbortSignal.timeout(5000)});if(res.ok){const d=await res.json();if(d.routes?.[0]?.geometry?.coordinates){const lc=d.routes[0].geometry.coordinates.map(c=>[c[1],c[0]]);map.removeLayer(rLine);rLine=L.polyline(lc,{color:accent,weight:tripActive?7:4,opacity:tripActive?0.98:0.9,lineCap:'round',lineJoin:'round'}).addTo(map);if(tripActive) map.fitBounds(rLine.getBounds(),{paddingTopLeft:[40,40],paddingBottomRight:[40,230]});if(d.routes[0].legs){const activeLeg=d.routes[0].legs[0];if(activeLeg){routeStops[0].tt=Math.ceil(activeLeg.duration/60);nsDist=((activeLeg.distance||0)/1000).toFixed(1)+'km';nsEta=fmtM(routeStops[0].tt);}}const ns=d.routes[0].legs[0]?.steps?.find(step=>step?.name||step?.maneuver?.modifier||step?.maneuver?.type);if(ns){const road=ns.name?` via ${ns.name}`:'';const action=(ns.maneuver?.modifier||ns.maneuver?.type||'continue').replace(/_/g,' ');const navText=`Next: ${action}${road}`.trim();document.getElementById('nav-turn').textContent=navText;maybeSpeakNavInstruction(navText);}document.getElementById('nav-dist').textContent=nsDist;document.getElementById('nav-eta').textContent=nsEta;applyMapHeadingRotation();}}}catch(e){}
  ...
  availableToSelect.forEach(loc => {
    ...
    item.innerHTML = `
      ...
      <div style="flex:1;display:flex;flex-direction:column;">
        <span style="font-weight:600;font-size:15px;color:var(--text-primary);">${loc.name}</span>
