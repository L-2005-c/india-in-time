...
const STATIC_ACTIONS = {
  dismissSplash, toggleSplashSound,
...
function updateItinUI(){
  ...
  itin.forEach((loc,i)=>{
    ...
    const whyNowBox = (loc.whyNow?.reasons?.length)
      ? `<div class="why-now-card" style="margin-top:5px;padding:4px 8px;background:rgba(56,189,248,0.08);border-left:3px solid var(--brand,#38bdf8);border-radius:4px;font-size:10.5px;"><strong style="color:var(--brand,#38bdf8);">${escapeHtml(loc.timePhaseBadge || loc.whyNow.timePhase || '✨ Why Now?')}</strong><div style="margin-top:2px;color:var(--text-main);">${loc.whyNow.reasons.slice(0, 2).map(r => `• ${escapeHtml(r)}`).join('<br>')}</div></div>`
      : whyTimeNote;
    ...
    if(nextStop && !nextStop.isBreak){
      const nextKm = (loc.coords && nextStop.coords) ? Math.round(hvKm(loc.coords[0], loc.coords[1], nextStop.coords[0], nextStop.coords[1]) * 1.42 * 10) / 10 : null;
