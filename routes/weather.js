...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const result = {
      ...
      classification: truth.classification || (isSeasonal ? 'HISTORICAL' : (truth.dataState === 'OBSERVED' ? 'OBSERVATION' : 'FORECAST')),
      isObservation: Boolean(truth.isObservation ?? (truth.dataState === 'OBSERVED')),
      isForecast: Boolean(truth.isForecast ?? (!truth.isObservation && !isEstimated)),
      isEstimated,
      forecastHorizonHours: truth.isObservation ? 0 : 1,
      ...
      temporalGrounding: {
        forecastIssuedAt: truth.timestamps?.issuedAt || null,
        forecastTargetTime: truth.forecastFor || truth.observedAt || new Date().toISOString(),
        observationTime: truth.observedAt || null,
        timeDeltaMinutes: truth.observedAt ? Math.round(Math.abs(Date.now() - new Date(truth.observedAt).getTime()) / 60000) : null,
        validity: truth.dataState === 'OBSERVED' ? 'OBSERVATION_GROUND_TRUTH' : (isEstimated ? 'ESTIMATED_MODEL' : 'NWP_FORECAST'),
      },
      spatialGrounding: {
        targetCoords: { lat: numLat, lon: numLon },
        targetElevationM: numElev ?? truth.station?.elevationM ?? null,
        stationCoords: truth.station ? { lat: truth.station.lat, lon: truth.station.lon } : null,
        stationElevationM: truth.station?.elevationM ?? null,
        distanceKm: truth.station?.distanceKm ?? null,
        elevationDeltaM: (numElev != null && truth.station?.elevationM != null) ? Math.round(numElev - truth.station.elevationM) : 0,
      },
